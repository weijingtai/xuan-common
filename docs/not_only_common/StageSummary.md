...
- `xuan-four-zhu-editor` 已新增 `EditableFourZhuStyleEditorPanel` 本地 facade 入口，并接入 style panels barrel export / entrypoints contract（`65871a6`）。
...
- `xuan-common` 里 `card_layout_model.dart` 已切到 `xuan-four-zhu-card` facade，继续缩小旧 layout model 真实现残留（`c80dd19`）。
