...
- [ ] 冻结四柱 Card 首批 package 边界
- [ ] 冻结 `common` 在过渡期只做 facade 的原则
- [ ] 记录当前已知的边界污染点
...
- [ ] 盘点四柱 Card 相关源码入口
- [ ] 盘点四柱 Card 当前对 `common` 数据库/模板/同步层的依赖
- [ ] 盘点当前真实业务接入点（至少 `qizhengsiyu`）
- [ ] 标记哪些 import 属于非四柱专属泄漏依赖
...
- [ ] `xuan_four_zhu_card` 当前仍依赖 `xuan-common`
- [ ] `common` 因此还不能直接反向 re-export 新 package

需要完成：

- [ ] 识别第一批可脱离 `common` 的四柱专属基础类型
- [ ] 优先迁移 theme/style/model 最小子集
- [ ] 确保新 package 对 `common` 的依赖逐步缩窄
- [ ] 为后续 `common` facade 预留无环依赖路径
...
- [x] 添加最小 `enums` 门面
- [ ] 机械迁移 `style_resolver.dart`
- [ ] `style_resolver.dart` 的阻塞点：依赖 `layout_template.dart`，本轮不把模板层一起迁出
- [ ] 继续收拢 `editable_fourzhu_card_impl.dart` 的残余 `common` 依赖
- [ ] 当前仅允许机械迁移，不改业务逻辑
...
- [ ] 迁移 `EditableFourZhuCardV3` 相关实现到 `xuan_four_zhu_card_ui`
- [ ] 迁移 card theme 与相关 style model
- [ ] 迁移 card 内部 cells / decorators / size calculator
- [ ] 在 `common` 中保留旧导出兼容入口

迁移前必须额外检查：

- [ ] `EditableFourZhuCardV3` 中直接引用的 `package:common/...` 类型清单
- [ ] 哪些依赖属于四柱专属，哪些属于更底层基础类型
- [ ] 是否会因为反向 facade 形成循环依赖
...
- [ ] 迁移 `FourZhuCardHost`
- [ ] 迁移 `FourZhuCardHostResolver`
- [ ] 清理 host 中非四柱专属 capsule/settings 依赖
- [ ] 固定 host 对 editor 的边界
- [ ] 固定 host 对平台模板能力的临时依赖方式
...
- [ ] 将旧路径改为 re-export 或薄 adapter
- [ ] 禁止继续在 facade 中新增真实实现
- [ ] 标记所有迁移完成后的旧入口状态

验收：

- [ ] 旧 app 仍可工作
- [ ] 新 app 可直接依赖 feature package

---

## 7. 业务试点：`qizhengsiyu`

- [ ] 识别 `qizhengsiyu` 当前四柱接入点
- [ ] 优先切宿主展示入口到新 package
- [ ] 保持编辑页入口正常
- [ ] 验证 fallback 能力在独立业务中仍成立
